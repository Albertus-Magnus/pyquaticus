...
if __name__ == '__main__':
    ...
    ####################markerstart
    #print("Action Space Type:", type(act_space))  # Debugging line to check action space type
    #if not hasattr(act_space, 'n'):
    #    raise ValueError("Action space must be of type gym.spaces.Discrete") 
    # #TODO#is of this type. CQL seems to expect a different type of action space, perhaps a Box space(?!). This might be the source of the error. The action space of the environment may need to be modified to be compatible with CQL, or a custom policy may need to be implemented to handle the current action space.
    # the above thought of a box space might be wrong since CQL "low" expects a non-discrete (what is the word for float?) space where there is a minimum value (or tbp. a value range).
    ####################markerend
    ...
    #ppo_config = CQLConfig().api_stack(enable_rl_module_and_learner=False, enable_env_runner_and_connector_v2=False).environment(env='pyquaticus').env_runners(num_env_runners=1, num_cpus_per_env_runner=1)
    ...
    #print(config.to_dict()) #comment this?
    print("marker 4--")    # Success this far, the error only shows after this point. -> The error is likely in the algo building or training step. <- changed api to old version
